...
public class EditorContronler : MonoBehaviour
{
    ...
    void GetColumnAndRow(GridLayoutGroup glg, out int column, out int row)
    {
        ...
        for (int i = 1; i < glg.transform.childCount; i++)
        {
           ...
           GetChild(i).GetComponent<RectTransform>();
            Word _scriptWord = glg.transform.
            GetChild(i).GetComponent<Word>();
    ...
    public void Test()
    {
        ...
        int row = 0;
        for (int column = 0; column < parentTransform.childCount; column++)
        {

            Word _scriptWord = parentTransform.GetChild(column).GetComponent<Word>();
            if (column < 7)
            {
                _scriptWord.SetIndex(column, row);
            }
